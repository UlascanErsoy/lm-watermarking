...
class WatermarkBase:
    ...
    def _get_greenlist_ids(self, input_ids: torch.LongTensor) -> list[int]:
        ...
        print("HIYA", input_ids.device)
        vocab_permutation = torch.randperm(self.vocab_size, generator=self.rng, device=input_ids.device)
...
class WatermarkDetector(WatermarkBase):
    ...
    def detect(
        ...
        **kwargs,
        ...
        # call score method
        output_dict = {}
        score_dict = self._score_sequence(input_ids=tokenized_text, prefix_len=len(tokenized_prefix), **kwargs)
        if return_scores:
        ...
        if return_prediction:
            ...
            if score_dict.pop("invalid", False):
                output_dict.update({"invalid": True})
                return output_dict
